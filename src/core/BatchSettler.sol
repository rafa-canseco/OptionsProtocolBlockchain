...
contract BatchSettler is Initializable, UUPSUpgradeable, ReentrancyGuard, IFlashLoanSimpleReceiver {
    ...
    uint    private _cachedChainId;

    struct Quote {
        address oToken;
        uint bidPrice; // premium per oToken (1e8 scale, in strike asset units)
        uint deadline; // unix timestamp
        uint quoteId; // unique per MM, for fill tracking + cancellation
        uint maxAmount; // max oTokens fillable (8 decimals)
        uint makerNonce; // must match current makerNonce[signer]
    }
    ...
    uint private constant CANCEL_BIT = 1 << 255;
    ...
    uint    public protocolFeeBps; // basis points (400 = 4%, max 2000 = 20%)
    ...
    mapping(address => mapping(bytes32 => uint)) public quoteState;
    /// @notice Global nonce per MM for bulk cancellation (circuit breaker).
    mapping(address => uint) public makerNonce;
    ...
    mapping(address => mapping(address => uint)) public mmOTokenBalance;
    ...
    mapping(address => mapping(uint => address)) public vaultMM;

    /// @notice Delay after expiry before MM can self-redeem (escape hatch).
    uint public escapeDelay;
    ...
    event VaultSettleFailed(address indexed vaultOwner, uint vaultId, bytes reason);
    event RedeemFailed(address indexed oToken, uint amount, bytes reason);
    event OperatorUpdated(address indexed oldOperator, address indexed newOperator);
    event PhysicalDelivery(address indexed oToken, address indexed user, uint contraAmount, uint collateralUsed);
    event PhysicalRedeemFailed(address indexed oToken, address indexed user, uint amount, bytes reason);
    ...
    event MakerNonceIncremented(address indexed mm, uint newNonce);
    event MMWhitelisted(address indexed mm, bool status);
    event MMSelfRedeem(address indexed mm, address indexed oToken, uint amount, uint payout);
    event EscapeDelayUpdated(uint oldDelay, uint newDelay);
    event MMBalanceCleared(address indexed mm, address indexed oToken, uint amount);
    event ProtocolFeeBpsUpdated(uint oldFeeBps, uint newFeeBps);
    ...
    function setProtocolFeeBps(uint _feeBps)
        external
        onlyOwner
    {
    ...
    function setSwapFeeTier(uint24 _feeTier)
        external
        onlyOwner
    {
    ...
    uint public constant MIN_ESCAPE_DELAY = 3 days;

    function setEscapeDelay(uint _delay)
        external
        onlyOwner
    {
    ...
    function setWhitelistedMM(address _mm, bool _status)
        external
        onlyOwner
    {
    ...
    function hashQuote(Quote calldata quote)
        public
        view
        returns
        (bytes32)
    ...
    function getQuoteState(address mm, bytes32 quoteHash)
        external
        view
        returns (uint filledAmount, bool isCancelled)
    {
        uint state        = quoteState[mm][quoteHash];
    ...
    function cancelQuote(bytes32 quoteHash)
        external
    {
        uint state                             = quoteState[msg.sender][quoteHash];
    ...
    function cancelQuotes(bytes32[] calldata quoteHashes)
        external
    {
        for  (uint i               = 0; i < quoteHashes.length; i++)  {
            uint state                 = quoteState[msg.sender][quoteHashes[i]];
    ...
    function batchSettleVaults(address[] calldata owners, uint[] calldata vaultIds)
        external
        onlyOperator
    {
    ...
    function batchRedeem(address[] calldata oTokens, uint[] calldata amounts)
        external
    {
        ...
        external
    {
        ...
        uint    balBefore       = IERC20(collateralAsset).balanceOf(address(this));

        ctrl.redeem(oToken, amount);

        uint payout = IERC20(collateralAsset).balanceOf(address(this)) - balBefore;
        if (payout > 0)  {
        ...
        returns (bool)
    {
        ...
        (address oToken, address user, uint oTokenAmount, uint maxCollateralSpent, address mm) =
            abi.decode(params, (address, address, uint, uint, address));

        IERC20(asset).safeTransfer(user, amount);

        uint collateralUsed = _redeemAndSwap(oToken, oTokenAmount, asset, amount + premium, maxCollateralSpent, mm);
    ...
    function _redeemAndSwap(
        ...
        uint maxCollateralSpent,
        address mm
    ) private returns (uint collateralUsed)  {
        // CEI: decrement MM balance before external calls
        mmOTokenBalance[mm][oToken] -= oTokenAmount;

        Controller ctrl             = Controller(addressBook.controller());
        address    collateralAsset  = OToken(oToken).collateralAsset();
        uint       collateralBefore = IERC20(collateralAsset).balanceOf(address(this));

        // BatchSettler already holds oTokens — redeem burns from msg.sender (this)
        ctrl.redeem(oToken, oTokenAmount);

        uint collateralReceived  = IERC20(collateralAsset).balanceOf(address(this)) - collateralBefore;
        ...
        uint surplus = collateralReceived - collateralUsed;
        if (surplus > 0)  {
    ...
    function batchPhysicalRedeem(
        ...
        uint[] calldata maxCollateralSpents,
    ...
    function _executePhysicalRedeem(
        ...
        (uint  expiryPrice, bool isSet) = oracle.getExpiryPrice(ot.underlying(), ot.expiry());
        if (!isSet) revert ExpiryPriceNotSet();

        uint strike = ot.strikePrice();
        if (ot.isPut())  {
    ...
    function mmSelfRedeem(address oToken, uint amount)
        external
        nonReentrant
    {
        ...
        external
    {
        ...
        uint balance = mmOTokenBalance[mm][oToken];
        uint toClear = amount < balance ? amount : balance;
        if (toClear > 0)  {
    ...
    function verifyLedgerSync(address mm, address oToken)
        external
        view
        returns (uint ledgerBalance, uint actualBalance, bool inSync)
    {
    ...
    function _revertOnPanic(bytes memory reason)
        private
        pure
    {
        if (reason.length >= 4)  {
