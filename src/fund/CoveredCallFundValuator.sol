...
interface ICoveredCallChainlinkSpotFeed {
...
/// @notice Deterministic WETH-denominated valuation policy for one covered-call adapter version.
/// @dev Pre-expiry option liabilities require unique approved signed observations and fail closed without quorum.
contract CoveredCallFundValuator is IPositionValuator, ICoveredCallFundValuator {
    ...
    constructor(
        ...
        if (ICoveredCallChainlinkSpotFeed(spotFeed_).decimals() != spotFeedDecimals_) {
            revert InvalidSpotObservation();
        }
        spotFeed = spotFeed_;
    ...
    function _preExpiryLiability(
        ...
        if (liability > strategyPosition.collateral) liability = strategyPosition.collateral;
    }
    ...
    function _readSpot() private view returns (uint80 roundId, uint256 price, uint256 updatedAt) {
        ...
        (roundId, answer,, updatedAt, answeredInRound) = ICoveredCallChainlinkSpotFeed(spotFeed).latestRoundData();
        if (
