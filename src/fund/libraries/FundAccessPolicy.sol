...
library FundAccessPolicy {
    ...
    function coveredCallAdapterRules() internal pure returns (Rule[] memory rules) {
        rules = new Rule[](2);
        rules[0] = Rule(
            UPGRADE_TO_AND_CALL_SELECTOR, FundConstants.ADAPTER_UPGRADER_ROLE, FundConstants.ADAPTER_UPGRADE_DELAY
        );
        rules[1] = Rule(
            ICoveredCallFundAdapter.setAdapterConfig.selector, FundConstants.CURATOR_ROLE, FundConstants.CURATOR_DELAY
        );
    }
