...
///      that breaks when USDC amountOut exceeds 1e12 (>$1M at 6 decimals).
...
contract PhysicalRedeemFuzzTest is Test {
    ...
    /// @notice PUT ITM: user receives exactly amount * 1e10 WETH, settler holds nothing
    function testFuzz_physicalRedeem_putITM_amount(uint256 amount) public {
        ...
        // Settler holds no residual tokens
        assertEq(weth.balanceOf(address(settler)), 0);
        assertEq(usdc.balanceOf(address(settler)), 0);
    }

    /// @notice CALL ITM: user receives exactly (amount * strikePrice) / 1e10 USDC, settler holds nothing
    ...
    /// @notice PUT ITM: physicalRedeem succeeds for any ITM expiry price, correct delivery, no residual
    function testFuzz_physicalRedeem_putITM_expiryPrice(uint256 expiryPrice) public {
        // Upper bound leaves margin for Aave flash loan fee (5 bps on borrowed amount).
        // Near the strike, collateral from redeem barely covers the swap repayment,
        // and the flash loan premium tips it over → "Too much slippage" revert.
        ...
        // User received correct contra-amount
        assertEq(weth.balanceOf(alice), aliceWethBefore + amount * 1e10);
        // Settler holds no residual tokens
        assertEq(weth.balanceOf(address(settler)), 0);
        assertEq(usdc.balanceOf(address(settler)), 0);
    ...
    /// @notice CALL ITM: physicalRedeem succeeds for any ITM expiry price, correct delivery, no residual
    function testFuzz_physicalRedeem_callITM_expiryPrice(uint256 expiryPrice) public {
        // Lower bound leaves margin for Aave flash loan fee (5 bps on borrowed amount).
        // Near the strike, collateral from redeem barely covers the swap repayment,
        // and the flash loan premium tips it over → "Too much slippage" revert.
